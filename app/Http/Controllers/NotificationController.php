...
class NotificationController extends Controller
{
    ...
    function getUserNotifications(Request $req)
    {
        ...
        $result =  UserNotification::Where('user_id', '=', $req->user_id)
            ...
            ->whereHas('data_user', function ( $query) {
        ...
        foreach ($result as $item) {
            $item->note_read=0;
            $item->save();
            if(isset($item->data_user->interests)){
                 $interest=Interest::whereIn('id', explode(",", $item->data_user->interests))->get();
                 $item->data_user->interests  = $interest;
            }
    ...
    function get_notificationcount(Request $request)
    {
        $result =  UserNotification::Where('user_id', '=', $request->user_id)->Where('note_read', '=',1)->count();
        return json_encode([
                'status' => true,
