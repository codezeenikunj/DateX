    ...
    function fetchAllReport(Request $request)
    {
        ...
        foreach ($result as $item) {
            ...
            if ($item->user->is_block  == 0) {
                $block  =  '<a class=" btn btn-danger text-white block" rel=' . $item->user->id . ' >' . __('block') . '</a>';
            } else {
                $block  =  '<a class=" btn btn-success  text-white unblock " rel=' . $item->user->id . ' >' . __('unblock') . '</a>';
            }

            $data[] = array(
                $item->user->first_name.' '.$item->user->last_name,
                $item->report_by->first_name.' '.$item->report_by->last_name,
                $item->reason,
                $block
            );
        }
